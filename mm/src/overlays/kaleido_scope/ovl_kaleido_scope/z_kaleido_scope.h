...
// #region SO2H [Menu] Cursor parked in one of the two arms of the backwards-L quest bar
// (so2h_quest_bar.h). These are NOT valid inputs to the vanilla
// `j = (cursorSpecialPos * 4) - 32` infoPanelVtx index math, which only defines values for
// 10 and 11; every site doing that math is guarded to those two values.
#define PAUSE_CURSOR_QUEST_BAR_RIGHT 12
#define PAUSE_CURSOR_QUEST_BAR_BOTTOM 13
// #endregion

// #region SO2H [Menu] Direction of pause pages - so2h 6-page hexagon prism (60 deg
// spacing), mechanically generated by tools/gen_pause_geometry.py (STRICTRULES.md rule
// 20). Never hand-edit these values; regenerate and re-paste instead. Enum order (see
// z64pause_menu.h PauseMenuPage) must match the fixed L/R cycle order these encode.
#define PAUSE_ITEM_X (0)
#define PAUSE_ITEM_Z (-1)
#define PAUSE_ITEM_OOT_X (0.8660254f)
#define PAUSE_ITEM_OOT_Z (-0.5f)
#define PAUSE_MAP_X (0.8660254f)
#define PAUSE_MAP_Z (0.5f)
#define PAUSE_QUEST_X (0)
#define PAUSE_QUEST_Z (1)
#define PAUSE_MASK_X (-0.8660254f)
#define PAUSE_MASK_Z (0.5f)
#define PAUSE_EQUIP_OOT_X (-0.8660254f)
#define PAUSE_EQUIP_OOT_Z (-0.5f)
// #endregion
...
// #region SO2H [Menu] OOT Item / OOT Equip pages (hexagon prism faces 1 and 5).
// Implemented in mm/2s2h/Compat/Menu/kaleido_compat_menu.c. Currently stub-safe
// (no-op when the page's Vtx buffers aren't allocated yet) - real content lands in
// patches 0004 (OOT Item) and 0005 (OOT Equip, Link doll framebuffer).
