...
// Ported from reference/soh/soh/soh/resource/importer/TextFactory.cpp. See TextFactory.h for why
// this exists in env/2s2h even though nothing currently produces type "Text"/OTXT here.
namespace SOH {
std::shared_ptr<Ship::IResource> ResourceFactoryBinaryTextV0::ReadResource(
    std::shared_ptr<Ship::File> file, std::shared_ptr<Ship::ResourceInitData> initData) {
...
std::shared_ptr<Ship::IResource> ResourceFactoryXMLTextV0::ReadResource(
    std::shared_ptr<Ship::File> file, std::shared_ptr<Ship::ResourceInitData> initData) {
    if (!FileHasValidFormatAndReader(file, initData)) {
        return nullptr;
    }
